...
import sys
import glob
import webbrowser  # Added for opening plots in browser
...
def process_csv(args: Tuple[str, bool, bool]) -> Tuple[str, Optional[Tuple], Optional[float], Optional[dict]]:
    ...
    try:
        ...
        symbol = os.path.basename(csv_file).replace('.csv', '')  # Extract symbol for cleaner naming
        ...
        if plot:
            print(f"{Fore.CYAN}[INFO] Generating and displaying plot for {symbol}{Style.RESET_ALL}")
            ...
            # Panel 1: Price and Portfolio Value
            fig.add_scatter(
                x=df.index,
                y=df['close'],
                mode='lines',
            ...
            try:
                ...
                try:
                    ...
                    if plot:
                        webbrowser.open(f"file://{os.path.abspath(fallback_path)}")
