...
def process_csv(args: Tuple[str, bool, bool]) -> Tuple[str, Optional[Tuple], Optional[float], Optional[dict]]:
    """
    Process a CSV file for backtesting.

    Args:
        args (Tuple): Contains csv_file (str), plot (bool), use_lstm (bool).

    Returns:
        Tuple containing the CSV file path, best parameter key, best return, and portfolio dictionary.
    """
    csv_file, plot, use_lstm = args
    ...
    try:
        ...
        # Set index to timestamp for proper alignment
        ...
        # Save best parameters and return
        result_file = os.path.join(os.path.dirname(csv_file), f"results_{os.path.basename(csv_file)}.pkl")
        with open(result_file, 'wb') as f:
            pickle.dump({'best_key': best_key, 'best_return': best_return}, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plot", action="store_true", help="Generate and save Plotly HTML plots of the backtest results (default: False)")
    ...
    # Data directory from fetching script
    ...
    # Process CSV files sequentially to avoid nested multiprocessing
    results = []
    for csv_file in csv_files:
        result = process_csv((csv_file, args.plot, args.use_lstm))
